...
public class SodiumVertexFormats {
    ...
    public static final GlVertexFormat<ChunkMeshAttribute> CHUNK_MESH_FULL = GlVertexAttribute.builder(ChunkMeshAttribute.class, 32)
            .addElement(ChunkMeshAttribute.POSITION, 0, GlVertexAttributeFormat.FLOAT, 4, false)
            .addElement(ChunkMeshAttribute.COLOR, 16, GlVertexAttributeFormat.UNSIGNED_BYTE, 4, true)
            .addElement(ChunkMeshAttribute.TEXTURE, 20, GlVertexAttributeFormat.FLOAT, 2, false)
            .addElement(ChunkMeshAttribute.LIGHT, 28, GlVertexAttributeFormat.UNSIGNED_SHORT, 2, true)
            .build();
    ...
    static {
        registerEncoder(CHUNK_MESH_FULL, (quad, buffer, position, mipped) -> {
            for (int i = 0; i < 4; i++) {
        ...
        registerEncoder(CHUNK_MESH_COMPACT, (quad, buffer, position, mipped) -> {
            for (int i = 0; i < 4; i++) {
